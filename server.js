...
const {createProject,getProject,runProject,approveProject,subscribe}=require('./src/orchestrator');
...
async function api(req,res,url){
  ...
  const match=url.pathname.match(/^\/api\/projects\/([^/]+)(?:\/(run|approve|events))?$/);if(!match)return json(res,404,{error:{code:'NOT_FOUND',message:'route not found'}});
