...
const backupManager=new BackupManager(defaultStore);
...
async function api(req,res,url){
  if(req.method==='GET'&&url.pathname==='/api/health')return json(res,200,{status:'ok',service:'nhanes-research-agent',version:'2.21.1',mode:'agent-orchestrated-mvp',authEnabled:auth.enabled,shuttingDown});
  ...
  if(req.method==='GET'&&url.pathname==='/api/health/diagnostics'){const database=defaultStore.health(),backups=backupManager.status(),runtime={uptimeSeconds:Math.floor(process.uptime()),memoryMegabytes:Math.round(process.memoryUsage().rss/1024/1024),node:process.version},status=database.ok&&backups.ok&&!shuttingDown?'healthy':'degraded';return json(res,status==='healthy'?200:503,{status,database,backups,workload:defaultStore.stats(),runtime,shuttingDown})}
...
if(require.main===module){backupManager.start();server.listen(process.env.PORT||4173,()=>console.log('NHANES Lab: http://localhost:4173'));const shutdown=signal=>{if(shuttingDown)return;shuttingDown=true;backupManager.stop();console.log(JSON.stringify({event:'server.draining',signal,at:new Date().toISOString()}));server.close(()=>process.exit(0));setTimeout(()=>process.exit(1),25000).unref()};process.on('SIGTERM',()=>shutdown('SIGTERM'));process.on('SIGINT',()=>shutdown('SIGINT'))}
module.exports={server,backupManager};
