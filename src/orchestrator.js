...
function approveModelSpec(projectId, input = {}) {
  ...
  project.feasibility = { ...(project.feasibility || {}), status:'executable', supportedPipeline:'generic_survey_v3', blockers:[], message:'变量、周期、清洗规则、权重策略、缺失策略和通用 survey 模型均已冻结，可在最终确认方案后执行' };
  project.agentPlan = buildAgentPlan(project);
