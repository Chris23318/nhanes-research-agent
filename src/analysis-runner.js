...
function cancellationError() { const error = new Error('R analysis cancelled by user'); error.code = 'TASK_CANCELLED'; error.cancelled = true; return error; }
function executeR(args, cwd, timeoutMs, signal) { return new Promise((resolve, reject) => { if (signal?.aborted) return reject(cancellationError()); const child = spawn('Rscript', args, { cwd, stdio: ['ignore', 'pipe', 'pipe'], env: { PATH: process.env.PATH, HOME: '/tmp', LANG: 'C.UTF-8' } }); let logs = '', settled = false; const collect = chunk => { logs = (logs + chunk.toString('utf8')).slice(-16000); }, abort = () => { child.kill('SIGKILL'); finish(cancellationError()); }; const finish = (error, code) => { if (settled) return; settled = true; clearTimeout(timer); signal?.removeEventListener('abort', abort); error ? reject(Object.assign(error,{logs})) : code === 0 ? resolve(logs) : reject(Object.assign(new Error(`R exited with code ${code}`),{logs})); }; const timer = setTimeout(() => { child.kill('SIGKILL'); finish(new Error('R analysis exceeded the 10 minute limit')); }, Math.max(1, timeoutMs)); signal?.addEventListener('abort', abort, { once: true }); child.stdout.on('data', collect); child.stderr.on('data', collect); child.on('error', error => finish(error)); child.on('exit', code => finish(null, code)); }); }
async function run(job) {
  ...
  try {
    if (job.mode.startsWith('generic_survey_v')) {
      ...
      logs += await executeR([path.join(scriptDir,'prepare-data.R'),job.cacheDir,job.outputDir],scriptDir,deadline-Date.now(),job.abortController.signal);
      logs += await executeR([path.join(scriptDir,'model.R'),path.join(job.outputDir,'merged.rds'),job.outputDir],scriptDir,deadline-Date.now(),job.abortController.signal);
    } else logs += await executeR(['/app/runner/analysis.R',job.cacheDir,job.outputDir,String(job.cycles),String(job.ageMin)],process.cwd(),deadline-Date.now(),job.abortController.signal);
    if (!fs.existsSync(path.join(job.outputDir,'result.json'))) throw new Error('R completed without result.json');
    job.status = 'completed';
  } catch (error) { logs = `${logs}\n${error.logs || ''}`.slice(-16000); job.status = job.cancelRequested || error.cancelled ? 'cancelled' : 'failed'; job.error = job.status === 'failed' ? error.message : null; }
...
function startAnalysis(project) { safeId(project.id); const gate = executionGate(project); if (!gate.ready) { const error = new Error(gate.errors.join('; ')); error.status = 409; error.code = 'ANALYSIS_NOT_READY'; throw error; } if(gate.mode.startsWith('generic_survey_v')){const readiness=generateRProject(project).readiness;if(!readiness.ready){const error=new Error(readiness.errors.join('; '));error.status=409;error.code='ANALYSIS_NOT_READY';throw error}} const existing = jobs.get(project.id); if (existing && ['queued', 'running'].includes(existing.status)) return view(existing); const root = dataRoot(), job = { id: `run_${crypto.randomBytes(8).toString('hex')}`, projectId: project.id, project: structuredClone(project), mode:gate.mode, status: 'queued', gate, cacheDir: path.join(xptCacheRoot(), project.id), outputDir: path.join(root, 'results', project.id), cycles: project.intent.cycles.length, ageMin: project.intent.population?.ageMin || 18, startedAt: new Date().toISOString(), completedAt: null, error: null, cancelRequested: false, abortController: new AbortController() }; if (!fs.existsSync(job.cacheDir)) { const error = new Error('approved XPT data cache is missing'); error.status = 409; error.code = 'DATA_CACHE_MISSING'; throw error; } jobs.set(project.id, job); queue.push(job); setImmediate(drain); return view(job); }
...
module.exports = { executionGate, startAnalysis, getAnalysis, cancelAnalysis, getAnalysisArchive, getAnalysisReport, getAnalysisQuality };
