...
function resultTablesCsv(project, result) {
  ...
  if(result.imputationDiagnostics?.requested)for(const key of ['executed','m','maxit','seed','eligibleN','completeCaseN','imputedCellCount','loggedEventCount','fractionMissingInformation'])rows.push({section:'imputation_diagnostic',model:'multiple_imputation',term:key,label:key,estimate:result.imputationDiagnostics[key]});
