function selectCandidate(project, input = {}) {
  ...
  if (!['exposure', 'outcome'].includes(input.role)) fail('无效变量角色');
  const group = project.variableDiscovery?.candidates?.find(x => x.role === input.role);
  ...
  const selection = { role: input.role, variable: candidate.variable, file: candidate.file, description: candidate.description, cycles, reason: input.reason.trim(), selectedAt: new Date().toISOString(), status: 'codebook_review_required' };
  return selection;
}
