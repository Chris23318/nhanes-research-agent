...
class ProjectStore{
  constructor(filename=process.env.DATABASE_PATH||':memory:'){
    ...
    this.countProjects=this.db.prepare('SELECT COUNT(*) AS count FROM projects');
    this.countAudit=this.db.prepare('SELECT COUNT(*) AS count FROM audit_events');
    this.selectOperationalJobs=this.db.prepare('SELECT scope,data FROM jobs');
  }
  ...
  stats(){const jobs={};for(const row of this.selectOperationalJobs.all()){let status='unknown';try{status=String(JSON.parse(row.data).status||'unknown')}catch{}const scope=jobs[row.scope]||(jobs[row.scope]={total:0,statuses:{}});scope.total++;scope.statuses[status]=(scope.statuses[status]||0)+1}return{projects:Number(this.countProjects.get().count),auditEvents:Number(this.countAudit.get().count),jobs}}
