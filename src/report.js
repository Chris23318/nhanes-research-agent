...
function genericMarkdownReport(project,result){const main=(result.coefficients||[]).find(item=>item.term==='analysis_exposure'),spec=project.modelSpec||{},flow=result.flow||{},sensitivity=result.sensitivityCoefficients||[],weights=result.weightDiagnostics||{},design=result.designDiagnostics||{},missing=result.missingnessDiagnostics||[],completeCases=result.completeCaseDiagnostics||{},model=result.modelDiagnostics||{};return `# ${project.title||'NHANES 流行病学研究报告'}
...
| 变量 | 缺失 n | 缺失率 |
|---|---:|---:|
${missing.map(item=>`| ${item.variable} | ${item.missing_n} | ${number(item.missing_pct,1)}% |`).join('\n')||'| 未记录 | — | — |'}
