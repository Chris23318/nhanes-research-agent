...
function genericMarkdownReport(project,result){const main=(result.coefficients||[]).find(item=>item.term==='analysis_exposure'),spec=project.modelSpec||{},flow=result.flow||{},sensitivity=result.sensitivityCoefficients||[],weights=result.weightDiagnostics||{},design=result.designDiagnostics||{};return `# ${project.title||'NHANES 流行病学研究报告'}
...
| 模型 | 效应 | 95% CI | P |
|---|---:|---:|---:|
${sensitivity.map(item=>`| ${item.model} | ${number(effect(item))} | ${number(item.ci_low)}–${number(item.ci_high)} | ${number(item.p_value,4)} |`).join('\n')||'| 未执行 | — | — | — |'}
