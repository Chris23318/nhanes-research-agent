...
- 协变量：${(spec.covariates||[]).map(x=>`${x.variable}（${x.encoding}）`).join('、')||'未调整模型'}
...
主暴露效应=${number(effect(main))}（95% CI ${number(main.ci_low)}–${number(main.ci_high)}，P=${number(main.p_value,4)}；效应尺度：${main.effect_type}。
...
`;}
function genericHtmlReport(project,result){const main=(result.coefficients||[]).find(item=>item.term==='analysis_exposure'),spec=project.modelSpec||{},flow=result.flow||{};return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(project.title)} · NHANES 报告</title><style>body{font:15px/1.65 Arial,"Microsoft YaHei",sans-serif;color:#17211d;max-width:980px;margin:36px auto;padding:0 24px}h1,h2{color:#174f3b}.notice{background:#fbf3e7;padding:12px;border:1px solid #edddc4}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #dfe3dc;text-align:left}th{background:#edf3e3}code{background:#edf3ee;padding:2px 4px}figure{overflow:auto}</style></head><body><h1>${esc(project.title)}</h1><p>${esc(project.question)}</p><p class="notice">通用 survey R 分析已完成并通过自动质量门；仅描述横断面关联，不支持因果推断。</p><h2>数据与方法</h2><ul><li>周期：${esc((project.intent?.cycles||[]).join('、'))}</li><li>暴露：${esc((spec.exposureMappings||[]).map(x=>`${x.cycles?.[0]} ${x.variable}`).join('；'))}；转换 ${esc(spec.exposureTransform)}</li><li>结局：${esc((spec.outcomeMappings||[]).map(x=>`${x.cycles?.[0]} ${x.variable}`).join('；'))}；${esc(spec.outcomeFamily)} / ${esc(spec.outcomeTransform)}</li><li>权重：${esc(result.weightRule)}</li><li>协变量：${esc((spec.covariates||[]).map(x=>`${x.variable}（${x.encoding}）`).join('、')||'未调整模型')}</li></ul><h2>样本流程</h2><figure>${flowSvg(result)}</figure><p>合并 ${esc(flow.merged)} 条记录，最终完整案例 ${esc(flow.analytic_complete_case)}。</p><h2>主要结果</h2><p>主暴露效应 <b>${number(effect(main))}</b>（95% CI ${number(main.ci_low)}–${number(main.ci_high)}，P=${pvalue(main.p_value)}；${esc(main.effect_type)}）。</p><figure>${forestSvg(result)}</figure><h2>局限性</h2><ul>${(result.warnings||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul><h2>复现信息</h2><p>模型配置 <code>${esc(result.modelSpecDigest)}</code><br>${esc(result.runtime?.rVersion)}；survey ${esc(result.runtime?.surveyVersion)}；haven ${esc(result.runtime?.havenVersion)}；${esc(result.runtime?.completedAt)}</p></body></html>`;}
function markdownReport(project, result) { if(result.analysisMode==='generic_survey_v1')return genericMarkdownReport(project,result);const main = exposureRow(result), sensitivity = sensitivityRows(result), flow = result.flow || {}, decisions = approvalDecisions(project); return `# ${project.title || 'NHANES 流行病学研究报告'}
