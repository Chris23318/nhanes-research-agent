...
function evaluateGenericResult(result = {}) {
  ...
  const strictSurvey=['generic_survey_v2','generic_survey_v3','generic_survey_v4','generic_survey_v5','generic_survey_v6','generic_survey_v7'].includes(result.analysisMode),strictDiagnostics=['generic_survey_v3','generic_survey_v4','generic_survey_v5','generic_survey_v6','generic_survey_v7'].includes(result.analysisMode),strictDescriptives=['generic_survey_v4','generic_survey_v5','generic_survey_v6','generic_survey_v7'].includes(result.analysisMode),strictDomain=['generic_survey_v5','generic_survey_v6','generic_survey_v7'].includes(result.analysisMode),isAdvanced=['generic_survey_v6','generic_survey_v7'].includes(result.analysisMode),isV7=result.analysisMode==='generic_survey_v7',completeCases=result.completeCaseDiagnostics||{},model=result.modelDiagnostics||{},domain=result.domainDiagnostics||{};
  ...
  const imputation=result.imputationDiagnostics||{},miModel=sensitivity.find(item=>item.model==='multiple_imputation'),imputationValid=!isV7||(imputation.requested===true&&imputation.role==='sensitivity_analysis'&&imputation.method==='mice_chained_equations'&&Number.isInteger(Number(imputation.m))&&Number(imputation.m)>=5&&Number(imputation.m)<=50&&Number.isInteger(Number(imputation.maxit))&&Number(imputation.maxit)>=5&&Number(imputation.maxit)<=50&&Number.isInteger(Number(imputation.seed))&&Number(imputation.seed)>0&&imputation.outcomeImputed===false&&imputation.exposureImputed===false&&Number(imputation.eligibleN)>=analytic&&Number(imputation.completeCaseN)===analytic&&Number(imputation.imputedCellCount)>=0&&((imputation.executed===true&&Boolean(miModel)&&validCoefficient(miModel))||(imputation.executed===false&&imputation.reason==='no_missing_covariate_values'&&Number(imputation.imputedCellCount)===0)));
  const checks = [
    check('result_status','error',result.status === 'completed','分析执行状态必须为 completed',{value:result.status}),
    ...
    ...(isAdvanced?[check('nonlinear_analysis','error',nonlinearValid,'如预设限制性立方样条，必须完成收敛的 survey Wald 非线性检验',nonlinear),check('subgroup_analysis','error',subgroupValid,'如预设亚组，每个输出层必须至少30人并报告交互 P 值',{requested:advanced.subgroupCount,rows:subgroups.length})]:[]),
    ...(isV7?[check('multiple_imputation','error',imputationValid,'多重插补必须仅用于已冻结协变量敏感性分析，采用固定随机种子并按 Rubin 法则合并',imputation)]:[]),
    check('runtime_provenance','error',Boolean(result.runtime?.rVersion&&result.runtime?.completedAt),'必须记录 R 版本和完成时间',result.runtime||{})
  ];
  const failed=checks.filter(item=>item.status==='failed').length,warnings=checks.filter(item=>item.status==='warning').length;
  return {schemaVersion:isV7?'1.6':'1.5',status:failed?'failed':'passed',checkedAt:new Date().toISOString(),summary:{total:checks.length,passed:checks.length-failed-warnings,failed,warnings},checks};
}
