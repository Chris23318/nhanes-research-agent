...
- 可在网页中配置协变量多重插补的 `m`、`maxit` 和随机种子；完整案例保持主分析，插补仅作为 Rubin 法则合并的敏感性分析，暴露和结局不自动插补
...
- 网页、PDF 与 Word 报告包含 30 秒执行摘要、关键指标卡、通俗方法表、样本流程图、主效应森林图、亚组森林图和缺失率图，并逐项解释主模型、敏感性分析、非线性、交互、缺失数据和模型诊断；另提供证据强弱平衡表和常见问题，结论层强制区分统计学证据、临床意义与因果推断
- 研究方案导出与响应式布局
- 研究项目创建、运行、查询和确认 API
- SSE 实时 Agent 事件流
- 结构化领域校验、显式状态机与人工质量门
