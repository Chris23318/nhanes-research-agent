...
test('generic continuous survey result allows negative beta confidence limits',()=>{const result={analysisMode:'generic_survey_v1',status:'completed',weightRule:'WTMEC2YR / 1',flow:{merged:100,population_eligible:90,analytic_complete_case:80},coefficients:[{term:'analysis_exposure',effect:-0.5,ci_low:-0.8,ci_high:-0.2,p_value:0.01,effect_type:'beta'}],sensitivityCoefficients:[],modelSpecDigest:'a'.repeat(64),runtime:{rVersion:'R 4',completedAt:'now'}};const quality=evaluateResult(result);assert.equal(quality.status,'passed');assert.equal(quality.summary.warnings,1);});
