...
test('health endpoint exposes the deployed application version',async()=>{const response=await fetch(`${base}/api/health`);assert.equal(response.status,200);const body=await response.json();assert.equal(body.mode,'agent-orchestrated-mvp');assert.equal(body.version,'2.2.0')});
...
test('project lifecycle reaches the approval gate',async()=>{
  ...
  assert.equal(completed.status,'awaiting_approval');assert.equal(completed.variables.length,10);assert.equal(completed.feasibility.status,'executable');assert.equal(completed.literature.mode,'disabled');assert.match(completed.literature.query,/NHANES/);
