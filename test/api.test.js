...
test('health, readiness and diagnostics endpoints expose safe operational state',async()=>{let response=await fetch(`${base}/api/health`);assert.equal(response.status,200);const body=await response.json();assert.equal(body.mode,'agent-orchestrated-mvp');assert.equal(body.version,'2.26.1');assert.equal(body.authEnabled,false);assert.equal(body.shuttingDown,false);assert.equal(response.headers.get('x-frame-options'),'DENY');assert.match(response.headers.get('content-security-policy'),/script-src 'self'/);response=await fetch(`${base}/api/health/ready`);assert.equal(response.status,200);const ready=await response.json();assert.equal(ready.status,'ready');assert.equal(ready.database.ok,true);response=await fetch(`${base}/api/health/diagnostics`);assert.equal(response.status,200);const diagnostics=await response.json();assert.equal(diagnostics.status,'healthy');assert.equal(diagnostics.backups.state,'disabled');assert.equal(diagnostics.offsiteBackups.state,'disabled');assert.equal(diagnostics.backups.lastChecksum,undefined);assert.equal(diagnostics.offsiteBackups.lastRequestId,undefined);assert.equal(typeof diagnostics.workload.projects,'number');assert.ok(diagnostics.runtime.uptimeSeconds>=0)});
test('web app includes authentication, monitoring, report exports and resumable Agent transitions',async()=>{const page=await fetch(base).then(response=>response.text()),script=await fetch(`${base}/app.js`).then(response=>response.text());assert.match(page,/id="authGate"/);assert.match(page,/id="loginForm"/);assert.match(page,/id="agentTransition"/);assert.match(page,/id="importStudy"/);assert.match(page,/id="backupStudy"/);assert.match(page,/id="serviceStatus"/);assert.match(page,/aria-live="polite"/);assert.match(page,/id="transitionMinimize"/);assert.match(script,/X-CSRF-Token/);assert.match(script,/forkStudy/);assert.match(script,/updateBackupControls/);assert.match(script,/updateServiceStatus/);assert.match(script,/health\/diagnostics/);assert.match(script,/analysis-report-pdf/);assert.match(script,/analysis-report-docx/);assert.match(script,/analysis-tables-download/);assert.match(script,/analysis-manuscript-download/);assert.match(script,/transitionCancel/);assert.match(script,/resilientFetch/);assert.match(script,/method:'DELETE'/);assert.match(script,/已恢复正在运行的研究任务/);assert.match(script,/服务器重启后已恢复此任务/);assert.match(script,/正在核验官方代码本/);assert.match(script,/正在验证 CDC 数据文件/);assert.match(script,/正在缓存 NHANES 数据/);assert.match(script,/正在执行 R survey 分析/);assert.match(script,/multiple_imputation/)});

test('sidebar keeps recent projects scrollable above the fixed profile area',async()=>{const styles=await fetch(`${base}/styles.css`).then(response=>response.text());assert.match(styles,/\.layout>aside\{[^}]*height:100vh[^}]*overflow:hidden/);assert.match(styles,/\.layout>aside nav\{[^}]*min-height:0[^}]*overflow-y:auto/);assert.match(styles,/\.profile\{[^}]*position:static[^}]*flex:0 0 auto/)});
...
test('backup endpoints stay closed while administrator authentication is disabled',async()=>{let response=await fetch(`${base}/api/projects/import`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});assert.equal(response.status,403);assert.equal((await response.json()).error.code,'AUTH_REQUIRED_FOR_BACKUP');response=await fetch(`${base}/api/projects/prj_1234567890abcdef/backup`);assert.equal(response.status,403);assert.equal((await response.json()).error.code,'AUTH_REQUIRED_FOR_BACKUP')});
