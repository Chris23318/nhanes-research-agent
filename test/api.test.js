...
test('health endpoint exposes version and browser security headers',async()=>{const response=await fetch(`${base}/api/health`);assert.equal(response.status,200);const body=await response.json();assert.equal(body.mode,'agent-orchestrated-mvp');assert.equal(body.version,'2.11.0');assert.equal(response.headers.get('x-frame-options'),'DENY');assert.match(response.headers.get('content-security-policy'),/script-src 'self'/)});
test('web app includes resumable Agent waiting transitions for heavy tools',async()=>{const page=await fetch(base).then(response=>response.text()),script=await fetch(`${base}/app.js`).then(response=>response.text());assert.match(page,/id="agentTransition"/);assert.match(page,/aria-live="polite"/);assert.match(page,/id="transitionMinimize"/);assert.match(script,/beginAgentTransition/);assert.match(script,/updateAgentTransition/);assert.match(script,/endAgentTransition/);assert.match(script,/已恢复正在运行的研究任务/);assert.match(script,/正在核验官方代码本/);assert.match(script,/正在验证 CDC 数据文件/);assert.match(script,/正在缓存 NHANES 数据/);assert.match(script,/正在执行 R survey 分析/)});
