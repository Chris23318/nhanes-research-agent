...
test('health and readiness endpoints expose safe operational state',async()=>{let response=await fetch(`${base}/api/health`);assert.equal(response.status,200);const body=await response.json();assert.equal(body.mode,'agent-orchestrated-mvp');assert.equal(body.version,'2.20.0');assert.equal(body.authEnabled,false);assert.equal(body.shuttingDown,false);assert.equal(response.headers.get('x-frame-options'),'DENY');assert.match(response.headers.get('content-security-policy'),/script-src 'self'/);response=await fetch(`${base}/api/health/ready`);assert.equal(response.status,200);const ready=await response.json();assert.equal(ready.status,'ready');assert.equal(ready.database.ok,true)});
