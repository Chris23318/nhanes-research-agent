...
test('health endpoint exposes the deployed application version',async()=>{const response=await fetch(`${base}/api/health`);assert.equal(response.status,200);const body=await response.json();assert.equal(body.mode,'production-mvp');assert.equal(body.version,'2.0.0')});
