...
test('completed R results download as an audited archive', () => {
  ...
  try { const archive = getAnalysisArchive({ id: projectId, title: 'Test', question: 'Test?', intent: { cycles: ['2017-2018'], population: { ageMin: 20 } } }); assert.equal(archive[0], 0x1f); const tar = zlib.gunzipSync(archive).toString('utf8'); assert.match(tar, /result\.json/); assert.match(tar, /result-manifest\.json/); assert.match(tar, /REPORT\.zh-CN\.md/); assert.match(tar, /forest-plot\.svg/); }
